...
public class MusicService extends Service implements OnPreparedListener, OnCompletionListener {
	...
	public void onCreate() {
		...
		// 将所有的状态加入到过滤器中
		intentFilter.addAction(Config.ACTION_PLAY);// 播放
		intentFilter.addAction(Config.ACTION_PAUSE);// 暂停
		intentFilter.addAction(Config.ACTION_NEXT);// 下一首
		intentFilter.addAction(Config.ACTION_LAST);// 上一首
		intentFilter.addAction(Config.ACTION_LIST);// 点击listView
		intentFilter.addAction(Config.ACTION_LIST_SEARCH);// 单机搜索界面的listView条目
		intentFilter.addAction(Config.ACTION_PLAN_CURRENT);// 用来发送进度条位置给服务的意图
	...
	public void initNotification() {
		...
		mNotification.tickerText = "欢迎使用桂涛音乐！";
		// 创建自定义通知视图
